...
#include <asn1_hsmclient/ECCPublicKey.h>
...
)
{
    ...
    {
        // I am assuming ndevs 
        while ((ndevs < 10) && (devices[ndevs] != NULL))
        {
            ndevs++;
        }
        if (ndevs >= 10)
	...
	DirectIOInput input) // input for the command.
{
	...
	// Need to remove logging to stdout or put in some other logging here instead.
	...
	UIntInput input) // input for command.   Desired Generation
{
	...
	if (retVal.returnCode == ERP_ERR_NOERROR)
	{
		...
		// TODO Add asn_check_constraints here...
